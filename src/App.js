...
function App() {
  ...
  if (mode === "MAIN") {
    ...
    content = (
      <MOOD setMode={setMode} setCate={setCate} setMood={setMood}></MOOD>
    );
    ...
    <>
      <div className="main_form">
        ...
        <div className="find_nearby">
          ...
          <img
            className="nearby_img"
            alt="nearbyimg"
            src="img/nearby_img.png"
          />
        </div>
...
function random_menu() {
  ...
  for (let i = 1; i <= 33 + chosen; i++) {
    setTimeout(function () {
      if (i === 33 + chosen) {
        container.innerHTML = "오늘의 메뉴는... " + menu_list[i % 11] + "!";
...
function MOOD(props) {
  ...
  const rendering = () => {
    ...
    return <div className="mood_wrapper">{result}</div>;
  };
